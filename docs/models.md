...
Use monthly subscriptions instead of per-token API billing. ChatGPT and Claude Code use Decepticon's `auth/*` custom handler so their CLI credential stores remain the source of truth; the others use custom LiteLLM handlers that authenticate via OAuth/session tokens.
...
| ChatGPT Pro/Plus/Team | `openai_oauth` | auth/gpt-5.5, auth/gpt-5.4 | Codex CLI `~/.codex/auth.json` via `auth` provider |
